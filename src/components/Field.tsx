...
export function TextInput(props: InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <input
      {...rest}
      className={`h-11 rounded-md bg-input/60 border border-border px-3 text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:ring-2 focus:ring-ring/60 ${className}`}
    />
  );
}
